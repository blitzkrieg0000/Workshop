...
# Regularizasyon yani düzenlileştirme, modelin eğitiminde hesaplanan loss değerlerine çeşitli değerler ekleyerek, modelin ağırlıklarını küçük tutmaya yarar.
# Overfitting in önüne geçmek için kullanılırlar.
# Ridge Regression (Tikhonov) -> Kullanılırsa ağırlıklar küçük tutulacaktır. 
#α-parametresi çok yüksek verilirse (α=10), ağırlıklar sıfıra çok yakın olacağından modelin tahmin doğrusu, veri seti ortalamasına yakınsayacaktır.(Düz çizgi)
#α-parametresi 10e-2 gibi bir değer verilirse, modelin overfitting olmamasına yardımcı olur.
# Lasso Regression -> Kullanılırsa bir nevi feature extraction yapılmış olur.
#Gereksiz küçük ayrıntıların elenmesini sağlayarak, ağırlıkları sıfır yapar.
#Bu regresyon kullanıldığında "sparse model" olarak adlandırılan çoğunlukla ağırlıkları 0 olan bir model ortaya çıkar.
# ElasticNet -> Ridge + Lasso karışımıdır.
#Karışım oranı r=0 ise Ridge, r=1 ise Lasso gibi davranır.
class ActivityRegularizationLayer(layers.Layer):
    def call(self, inputs):
        self.add_loss(1e-2 * tf.reduce_sum(inputs))
        return inputs

CustomRegularization = ActivityRegularizationLayer()
...
def train_step(x_batch_train, y_batch_train):
    with tf.GradientTape() as tape:
        ...
        #! Regularizasyon yöntemi gereği modeldeki Loss katmanlarından gelen fazladan Lossları bu forward için ölçülen Loss'a ekliyoruz.
